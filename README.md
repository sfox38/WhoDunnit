...
* **Robustness:** Switched the integration's internal timers and cache expiry to a monotonic clock, so they are no longer affected by system clock adjustments such as NTP corrections or daylight-saving changes. The human-facing `event_time` timestamp is unchanged.
